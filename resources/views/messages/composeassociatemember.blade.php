...
@section('content')
<?php
$fullmembers=count(DB::table('clients')->where(['member_type'=>'Associate'])->get());
$data=DB::table('clients')->where(['member_type'=>'Associate'])->get();
      ...
      @endif
          ...
          <div class="box box-primary">
            <div class="box-header with-border" style="background-color: #2bbbad !important;color:white;text-transform:uppercase;">
              <center><h3 class="box-title"> <a href="{{route('messages.inbox')}}"> <i class="fa fa-backward"></i>Go Back</a> &nbsp; &nbsp; Compose New Message [This message shall be received by {{$fullmembers}} Associate members] <a href="#" class="btn btn-link" data-toggle="modal" data-target="#modal-default">View Recipient</a></h3></center>

            </div>
            <form class="form-horizontal" method="POST" action="{{route('messages.sendToAssociate')}}">
           @csrf
